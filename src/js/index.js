(function() {
    ...
    function networkChanged(e) {
        ...
        var networkIndex = e?.target?.value || 0;
        ...
        function calculateValues() {
            setTimeout(function() {
              ...
              if (networks[DOM.network.val()].name == "EOS - EOSIO") {
                ...
                // TON
                if (networks[DOM.network.val()].name == "TON - The Open Network") {
                    var path = getDerivationPath() + "'/" + index + "'/0'";
                    bip32ExtendedKey = calcBip32ExtendedKey(path);

                    var privkeyBuffer = libs.ed25519.derivePath(path, seed).key;
                    privkey = privkeyBuffer.toString("hex");

                    var pubkeyBuffer = libs.ed25519.getPublicKey(privkeyBuffer, false);
                    pubkey = pubkeyBuffer.toString("hex");

                    // var key = bip32ExtendedKey.deriveHardened(index)
                    address = "(TODO bouncable WalletV4R2)";
                    // var accountXprv = accountExtendedKey.toBase58();
                    // var accountXpub = accountExtendedKey.neutered().toBase58();
                    // address = bip32ExtendedKey.deriveHardened(index).keyPair.toWIF();
                }
